...
**XRD reads a measured scan.** Every analysis surface in the 0.6.0 release took a simulated
pattern or a pasted column of numbers; the diffractometer output on a colleague's disk could not be
opened. It can now, and the same analyses run on it.

- **`pytex.adapters.xrdml.read_powder_xrdml`** reads a one-dimensional PANalytical XRDML powder
  scan, and **`pytex.diffraction.xrd_measurement.read_powder_pattern`** dispatches on the file
  itself, so a caller names a path rather than a format.

- **`xrd.background`, `xrd.lattice_parameters` and `xrd.rietveld`** gain a `scan_file` parameter and
  a `data_source` choice, declared once and shared, so what an *experimental scan* means cannot
  drift between the three views. `xrdscan.js` carries the loader in the panel.

- **Tracked synthetic fixtures** for a nickel fcc scan in both `.xy` and `.xrdml`, generated by
  `scripts/generate_xrd_test_patterns.py` with Caglioti broadening and a curved background, so the
  reader path is tested against something shaped like a real measurement rather than a clean
  triangle.

**The documentation is in the application, and works offline.** The workbench linked out to
GitHub-rendered MyST sources, which is useless on the air-gapped and intranet hosts the platform is
built for. `pytex.app.server` now serves `/docs/` from the built site with MathJax vendored, path
traversal refused, and a CSP that permits offline rendering; `DocumentationLink` emits in-app URLs,
and a Docs button sits in the masthead. The theory notes for TEM spot indexing, multi-spot
candidate picking, closed-form stage-tilt kinematics, and EBSD KAM and GND evaluation were written
out in full to have something worth serving.

**A form fits on one screen, and a control is as wide as its value.** The workbench spent most of
its width on nothing: a Miller index box that never holds more than three characters was given the
full rail, and three Bunge angles took three lines because `phi1`, `Phi` and `phi2` were the widest
thing on each of them. The labels were the layout, not the values, and the button was reached by
scrolling -- so the inputs of a calculation were never seen as the single statement they are. This
is now a cardinal rule in `AGENTS.md` and Decision 13 of the application platform document, and it
is declared on the parameter rather than applied panel by panel.

- **`pytex.core.symbols`** is the machine-readable half of the terminology and symbol registry: one
  entry per symbol, carrying the Unicode form a control shows and the mathtext body a figure axis
  takes. Naming an unregistered symbol raises at construction, so a misspelling fails the import
  instead of shipping a control labelled with a raw registry key. The table is served with the
  manifest as well, for the one control the frontend builds itself -- the phase control's cell
  editor, whose six boxes are the lattice parameters and were labelled with the words "alpha",
  "beta" and "gamma".

- **`Parameter` gains `symbol`, `row` and `field_width`.** The symbol is what a control *shows*
  while `label` remains what it is *called*, and is kept as the accessible name, the tooltip and
  the text of every error -- compactness is never bought by making a form unreadable to a newcomer
  or to a screen reader. `row` puts quantities that are entered together on one line. `field_width`
  states how much room a value needs in characters of content rather than pixels, and is usually
  omitted: a bounded number, a bounded integer and an indices row each derive their own.

- **What that came to.** 319 of 549 controls are narrower than the single default they all shared,
  92 parameters share a row, and 51 show a symbol. Counting only what a panel shows when it opens,
  the median operation now presents its whole form in 6 lines and 56 of 57 do so in 12 or fewer.

- **No mandatory input may hide behind a disclosure.** A required parameter with no default,
  placed behind Advanced or inside a collapsed group, makes a form that looks complete, fails on
  submission, and names a control that is not on screen. Checked across every operation, together
  with ten other rules of the layout, in `tests/unit/test_app_manifest.py`.

**Scientific behavior.** No change. The XRD reader adds an input path, the documentation server
adds a route, and the layout work changes no name, no default and no validation -- a client that
ignores all three of the new declarations renders exactly the form it rendered before.
