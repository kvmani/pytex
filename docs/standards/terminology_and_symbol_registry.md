...
| recalculated pole figure | The pole density an ODF predicts, evaluated on the directions of a measured figure. With the *difference pole figure* (recalculated minus measured) it is the acceptance test of a pole-figure inversion. |
| RP factor | Mean relative deviation, in percent, of a recalculated from a measured pole figure over the points measured above a threshold (0.5 m.r.d. in PyTex): $RP = \frac{100}{N}\sum \lvert P_{\mathrm{calc}} - P_{\mathrm{obs}}\rvert / P_{\mathrm{obs}}$. Reported against the inverted (symmetrized) figures and against the measured ones; the gap is the cost of the sample-symmetry assumption. |
| axial sample symmetry | The fibre (cylindrical) statistical symmetry of drawn, extruded or axially read product: invariance under every rotation about the fibre axis and the two-folds perpendicular to it, the Curie group $\infty/mm$. Imposed on a pole figure exactly, as a trapezoid-weighted azimuthal average on each ring of constant tilt. Synonyms accepted: fibre, fiber, cylindrical. |
| LaboTex plate | A presentation of an ODF as every section at a fixed step over the whole symmetry-reduced Euler range, on one shared scale. |
...
| EBSD camera geometry | The placement of a backscatter-diffraction camera stated the way it is configured: stage tilt, camera elevation and azimuth, pattern centre, and camera distance. Built by `DiffractionGeometry.for_ebsd`, which owns the frame convention so no caller composes the stage rotation and the camera placement by hand. |
| pattern centre | The point where the camera axis meets the screen, with the camera distance, as the triple $(x^{*}, y^{*}, z^{*})$: the first two fractions of the detector extent, the third the distance as a fraction of the detector **width**. Vendors differ over the origin and the axis the fractions are taken along, so a value copied from a vendor file is not necessarily in this frame. |
| stage tilt | The rotation of the specimen about the tilt axis in an EBSD geometry, tipping the specimen normal towards the camera. Seventy degrees is the standard working point: backscatter yield rises with tilt, and so do foreshortening and depth-of-field loss. |
...
| $\sigma$ | Stage tilt in an EBSD geometry, about the laboratory tilt axis. With the camera elevation $\epsilon$, the specimen normal projects at gnomonic radius $\tan(90^{\circ} - \sigma + \epsilon)$ — the identity that checks the whole convention. |
| $\epsilon$ | Elevation of the EBSD camera axis above the plane perpendicular to the beam. At $\epsilon = 0$ the beam lies in the screen plane and has no gnomonic image, which is correct: an EBSD camera never sees the incident beam. Distinct from $\varepsilon$, strain. |
