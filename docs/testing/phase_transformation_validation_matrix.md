...
| Map-scale parent-grain reconstruction (experimental v1) | Synthetic planted-parent recovery: three KS parents with five children each are re-grouped exactly (cross-parent edges rejected), parents recovered to 0 deg; 0.3-deg-noise case keeps the partition with residuals at the noise level. Multi-parent stress fixture: six randomly oriented parents whose cross-parent boundaries are asserted >8 deg from the fingerprint are partitioned exactly, where the previous angle-only edge test merged them | foundational | `pytex.experimental.reconstruct_parent_grains(...)`: full-rotation fingerprint edge test (axis and angle — see the row above; the earlier angle-only test is a fixed defect, recorded in the CHANGELOG as a scientific behavior change), union-find clustering, per-cluster candidate-parent scoring with quaternion-eigen-mean refinement; `describe()` states ambiguity caveats. Operating envelope characterized in [Reconstruction Robustness Study](reconstruction_robustness_study.md): zero false links across 48 sparse-adjacency cells (~700 trials) AND across the map-scale sweep, parent accuracy tracking sigma/sqrt(n), and a two-sided tolerance rule (>= 4x scatter, but <= ~1 deg for cubic-cubic KS on dense graphs). **Map-scale caveat:** at 100 tiled parents with four-connected adjacency the default 3.0 deg tolerance recovers only ~70 of 100 parents under KS (97.5 at 1.0 deg) because genuinely ambiguous cross-parent boundaries — not edge-test errors — merge parents; Burgers keeps 97 of 100 at 3.0 deg since its 12-variant fingerprint is far smaller. Results now report `chance_link_probability` and `describe()` warns when the expected chance-link count reaches one. Literature fixtures (martensite→austenite, alpha→beta Ti) and MTEX `calcParent2Child` parity remain ahead before stabilization; the parity campaign is defined but its MTEX side is ungenerated (no MATLAB available in the development environment). |
