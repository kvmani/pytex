...
| Physical properties (slip / Schmid) | public slip-system / Schmid-factor workflows | foundational | A first `pytex.properties` layer provides `SlipSystem`, symmetry-expanded `SlipSystemFamily` (12-system fcc `{111}<110>` and bcc `{110}<111>` families), vectorized absolute Schmid factors over orientation populations, and `CrystalMap.schmid_factor_map` for per-point maximum Schmid factor maps. Validated against the analytic `sqrt(6)/6` maximum Schmid factor for `[001]` uniaxial loading. A rank-4 elastic-tensor layer is now implemented too: `StiffnessTensor` / `ComplianceTensor` with 6x6 Voigt IO, cubic/hexagonal/isotropic constructors, 4th-rank rotation, compliance inversion, and directional Young's modulus, validated against the analytic cubic `1/E` formula and isotropy/rotation invariance. Orientation-weighted Voigt/Reuss/Hill polycrystal homogenization (`homogenize_elastic`) is implemented, validated by single-orientation reproduction of the crystal, near-isotropy of a random aggregate, and the Voigt > Hill > Reuss bound ordering. Full-constraint (Taylor) plasticity factors are also implemented (`taylor_factors`, `CrystalMap.taylor_factor_map`) via a SciPy HiGHS linear program with vectorized Schmid-tensor setup, validated against the exact cube-orientation value `sqrt(6)` and the classic random-fcc mean `~3.06`. Directional elastic-property surfaces are now available: batched (vectorized) directional Young's modulus and linear compressibility, plus `youngs_modulus_surface` / `DirectionalModulusSurface` (spherical sampling with anisotropy ratio) and a 3D `plot_youngs_modulus_surface`, validated against cubic extrema (`<100>`/`<111>`) and exact isotropy. The full standard set of directional elastic properties is now covered: directional shear modulus `G(n, m)` and directional Poisson's ratio `nu(n, m)` (batched, orthogonality-checked), with extremal `shear_modulus_surface` / `poisson_ratio_surface` computed exactly per direction via the in-plane 2x2 eigenproblem (no angular sweep), validated against the cubic shear constants (`c44` on `{100}<010>`, `(c11-c12)/2` on `{110}<-110>`), the cubic `-s12/s11` Poisson value, and exact isotropy. Full wave-velocity surfaces and rate-sensitive/relaxed-constraint models remain ahead. |
