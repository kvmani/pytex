...
| Generated algorithm figures | `tests/unit/test_figure_markers.py`: title/desc present, canonical font, absolute marker units, arrowhead scale, no text overflowing its card or the canvas, no colliding labels, and byte-for-byte reproducibility against the committed asset | implemented | Four flow sheets written by `scripts/generate_algorithm_figures.py` from `pytex.plotting.algorithm_diagrams`. Held to the same strict contract as the generated reference-frame figures, not only the marker check. |
| Figure generation determinism | `test_algorithm_figures_are_deterministic` regenerates in-process and compares with the committed bytes | implemented | Caught `builtins.hash` being used for arrowhead marker ids, which Python randomizes per process: regenerating a committed figure changed its bytes every run, so drift could not be detected. Now a `zlib.crc32` digest. |
| Shared SVG primitives | `pytex.plotting.svg_primitives` exercised through both generators; the reference-frame figures are byte-identical after the extraction | implemented | One definition of the style-guide tokens, text measurement and wrapping, cards, arrows, callouts, the non-scaling arrowhead marker, and the document wrapper that emits `<title>`/`<desc>`. |
| Pole-figure contour rendering | builder/runtime tests plus workbench interaction contracts | implemented | Contour pole figures are rendered from smoothed projected density grids; the shared app adds display-only inverse-distance resampling, marching-squares isolines, filled/line/combined modes, automatic or exact levels, scale/palette controls, and computed-sample hover targets. |
| ODF contour rendering | builder/runtime tests plus workbench interaction contracts | implemented | Euler-space section grids support the same declared isolines and filled bands in the shared desktop/web workbench, with adjustable levels and presentation properties. |
...
| Kikuchi pattern plotting | `tests/unit/test_kikuchi_pattern_plotting.py`: band naming checked against the geometry the same figure drew | implemented | Bands, Kossel-cone edges and zone axes in gnomonic or detector coordinates. `label_bands` writes each `(hkl)` along its own band; the test pairs every label with the nearest drawn trace and compares the rendered rotation with that trace's local tangent, after a draw — with `transform_rotates_text`, a rotation queried before layout is an angle nobody renders with. |
...
| 3D crystal visualization | scene-builder tests, workbench interaction contracts, and runtime figure generation | implemented | Unit-cell atoms, heuristic bonds, repeated cell overlays, hexagonal-prism overlays, bounded plane overlays, direction overlays, and scientific Miller annotations are covered. The shared workbench additionally checks reusable sphere gradients, cylindrical bond glyphs, glossy/matte/flat finishes, adjustable camera-relative lighting and depth cue, and faithful appearance transfer to publication export. |
