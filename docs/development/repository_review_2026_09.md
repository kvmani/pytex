...
| Core architecture | `core.parent_reconstruction` imports `experimental.phase_transformation.score_parent_orientations`. This is still present, despite the old guide identifying it as a priority. | Separate shared implementation from the experimental public facade in a subsequent increment. This does not justify promoting map reconstruction to stable status. |
...
| Diffraction | XRD fitting, phase identification, SAED, CBED and recent HREM code exist. HREM tests assumed optional ASE/abTEM were installed; the dependency declaration does not require them. | Test capability detection independently of the machine and run optional integration checks with ASE/abTEM installed. Preserve the distinction between phase-object and multislice models. |
