...
| How will the microscope optics change HREM contrast? | Atomic snapshot, beam energy, aberrations, coherence and sampling | {doc}`../theory/hrem_multislice_and_ctf`: transfer function and simulated image | The PyTex multislice ({doc}`../theory/multislice_hrtem`) is the default engine; record the engine, the sampling and its retained intensity, and the slice thickness before comparing simulated and measured contrast. |
| Which thickness and defocus does an experimental HRTEM image correspond to? | Periodic crystal slab, lens, a defocus range and several thicknesses | {doc}`../algorithms/multislice_hrtem`: focal series and defocus-thickness tableau from one exit wave | Match the image against the tableau, then refine; a few mrad of mistilt or an unconverged sampling can mimic a thickness change. |
