# Harmonic ODF Reconstruction in PyTex
...
pole figures (*Introduction to Texture Analysis*, pp.\ 105–107).
...
## Quadrature And Orthonormalization
...
Instead it uses the current measured-direction `PoleFigure` semantics and builds a
...
- Symmetry handling is explicit through the repository point-group operators, not a separate harmonic-specific symmetry abstraction.
- Negative ODF lobes can still appear because truncated harmonic expansions are not positivity preserving.
- Degree selection, quadrature density, and kernel halfwidth are scientific inputs, not hidden tuning constants.

## References

- V. Randle and O. Engler, *Introduction to Texture Analysis: Macrotexture, Microtexture and Orientation Mapping*, CRC Press, 2000.
