...
`pytex.tem`: given a crystal at a known holder position with an indexed zone axis, determine
...
*not* determine, and the bulk of this note is therefore devoted to that question.

## Frames

No new frame domain is introduced. Per the repository frame doctrine the holder frame *is*
the specimen-domain frame for TEM work, so the crystal-to-holder rotation is an ordinary
crystal-to-specimen `Orientation`.

- **Crystal** $C$: right-handed Cartesian tied to the lattice through the structure matrix $\mathbf{A}$, with $[uvw] \mapsto \mathbf{A}[u,v,w]^{\mathsf T}$ and $(hkl) \mapsto \mathbf{A}^{*}[h,k,l]^{\mathsf T}$, $\mathbf{A}^{*} = (\mathbf{A}^{-1})^{\mathsf T}$.
- **Holder** $H$: rigidly attached to the specimen and to the cartridge, with $\hat{\mathbf{x}}_H$ along the rod (the $\alpha$ axis), $\hat{\mathbf{y}}_H$ along the nominal $\beta$ axis. Coincides with the laboratory frame at zero tilt.
- **Laboratory** $L$: fixed to the column, $\hat{\mathbf{z}}_L$ pointing up the column toward the gun, so electrons propagate along $-\hat{\mathbf{z}}_L$.
- **Pattern** $P$: axes of the stored diffraction-image array, origin at the transmitted beam.
...
rotation $\mathbf{M}_{H \leftarrow S}$. **That rotation and the crystal orientation are not
...
in the column; the $\beta$ axis is a cradle carried *inside* the rod and therefore moves with
...
pair and *not* a general licence to ignore axis motion: it does not survive a non-orthogonal
...
Everything below is an inversion of, or a constraint on, {eq}`eq-tn-master`.

## Beam Direction, And Why It Is The Useful Form

Transposing {eq}`eq-tn-master` for the beam axis gives
...
This is a spherical coordinate system whose **pole is the $\beta$ axis**, and three
consequences follow immediately.

1. The Jacobian is $\cos\alpha$, so a tilt rectangle reaches the solid angle
...
2. Curves of constant $\beta$ are great circles through $\pm\hat{\mathbf{y}}_H$, and curves of constant $\alpha$ are small circles about it. The accessible region is a geodesic quadrilateral and draws exactly, as circular arcs.
3. $\lVert \partial\hat{\mathbf{b}}_H/\partial\alpha \rVert = 1$ but $\lVert \partial\hat{\mathbf{b}}_H/\partial\beta \rVert = \cos\alpha$: a degree of $\beta$ buys only $\cos\alpha$ degrees of crystal rotation. This is the double-tilt holder's gimbal lock.
...
and matching against {eq}`eq-tn-beam` gives
...
**The combinatorial richness of the answer therefore comes from symmetry and ambiguity, not
...
For a calibrated stage with a non-orthogonal or mis-set axis pair there is no closed form, and
 {eq}`eq-tn-solution` seeds a Gauss–Newton refinement on the two-dimensional residual.

## Reconstruction, And Why Two Zones Beat One Pattern
...
and two non-parallel correspondences determine a rotation uniquely (Wahba's problem). **This
...
## The Reverse Inversion: Orientation From An Indexed Pattern

Sections above invert {eq}`eq-tn-master` for the tilts. The same equation inverts for the
orientation, which is what texture and microstructure work requires of indexing. Given the
crystal-to-pattern rotation $\mathbf{R}_{P \leftarrow C}$ that indexing produces and the holder
angles at which the pattern was recorded, {eq}`eq-tn-mode-a` gives $\mathbf{U}$ directly, and
because the holder frame is the specimen-domain frame this is an ordinary crystal-to-specimen
orientation — reportable in Bunge angles and comparable with an EBSD measurement without further
conversion.

### Joint determination of orientation and diffraction rotation

The single-pattern inversion requires $\varphi_D$, and {eq}`eq-tn-residual-law` shows that an
error in it rotates the reported orientation bodily about the beam axis, degree for degree, with
crystal symmetry providing no protection. Two patterns remove the dependency entirely.

Let patterns $i = 1, 2, …$ be indexed at stage positions $(\alpha_i, \beta_i)$, with zone axes
$\hat{\mathbf{n}}_i$ and crystal-to-pattern rotations $\mathbf{R}_i$. The zone axes alone determine
$\mathbf{U}$ through {eq}`eq-tn-mode-b`, using no calibration. With $\mathbf{U}$ fixed, define

$$
\mathbf{M}_i \;\equiv\; \mathbf{R}_{\text{stage}}(\alpha_i,\beta_i)\,\mathbf{U}\,\mathbf{R}_i^{\mathsf T}.
$$ (eq-tn-residual)

Substituting {eq}`eq-tn-mode-a` shows $\mathbf{M}_i = \mathbf{F}\mathbf{R}_z(\varphi_D)$ for every
$i$. Two consequences follow, and they are independent:

1. **The diffraction rotation is read off, not fitted.** With $\mathbf{F} = \mathbf{I}$, $\varphi_D = \operatorname{atan2}\left((\mathbf{M}_i)_{21}, (\mathbf{M}_i)_{11}\right)$ exactly. The scatter of that value across patterns measures whether they agree on one instrument constant. Because $\varphi_D$ is an angle, the mean must be taken circularly: an arithmetic mean of $359^{\circ}$ and $1^{\circ}$ is $180^{\circ}$, which would be a spectacular way to report a calibration.
2. **The beam-axis deviation is a residual no calibration can absorb.** Every $\mathbf{M}_i$ must fix $\hat{\mathbf{z}}_L$. The angle $\angle(\mathbf{M}_i\hat{\mathbf{z}}_L, \hat{\mathbf{z}}_L)$ is therefore orthogonal to the entire $\varphi_D$ degree of freedom, and is non-zero only if an input is wrong — a mirrored stored pattern, a mis-indexed reflection, a reversed stage sign convention, or a bent specimen.

The second quantity is the operative diagnostic, and the reason a calibration derived this way may
be adopted with confidence: the data have already been asked whether they are consistent with
*any* value of the constant, not merely fitted to the best one.

### How a mirrored pattern presents

Not as an improper matrix, which is the natural but wrong expectation. Indexing constructs
right-handed orthonormal triads from both the observed and the calculated reflection pairs, so its
output is a proper rotation whatever the data. A mirrored recording instead reflects the stored
in-plane axes *and* reverses the sense of the derived pattern normal, since the normal is
built as a cross product of the reflected vectors. The two together compose to a $180^{\circ}$
rotation about the pattern $x$ axis, which is proper.

Mirroring therefore passes any determinant test and is detected only by the beam-axis deviation of
 {eq}`eq-tn-residual`. An improper matrix arriving at this surface means a mirror was composed in
by hand upstream, and is rejected as an input error rather than propagated.

## Identifiability: Three Independent Ambiguities
...
with $L$ the *Laue class*. The genuinely distinct hypotheses are counted by the cosets
...
**exactly ten of the thirty-two point groups** — those containing improper operations other
...
Two consequences. **For a centrosymmetric crystal Friedel's law adds nothing**: every
...
*non-centrosymmetric is the wrong test*: quartz, point group $32$, is acentric yet entirely
...
### Layer 2: Instrumental rotation

An error $\delta\varphi$ in the diffraction rotation is a rotation about the beam axis. It is
*not* absorbed by crystal symmetry, and following the tilts it implies lands the specimen
...
Equation {eq}`eq-tn-residual-law` carries the two operationally important facts. At
$\delta\varphi = 180^{\circ}$ both tilt angles are negated: the specimen goes exactly the wrong way
while the calculation reports a clean zero residual. And the error *scales with the length of
...
caught by $\det\mathbf{U}$ and by {eq}`eq-tn-consistency`.
...
## Eucentric Height Is Not An Orientation Parameter

Being off eucentric height introduces *no orientation error to first order*: tilting about a
displaced axis is a rotation composed with a translation, and only the rotation enters
 {eq}`eq-tn-master`. The real cost is that the region of interest translates out of the
...
**The mathematically optimal path is the one experienced operators already follow by eye**,
...
1. $\varphi_D$ wrong, worst at $180^{\circ}$: a clean, confident, exactly-backwards answer, detected only by calibration or by the two-zone path.
2. Parity or sign convention wrong: reflected trajectory, detected by $\det\mathbf{U}$ and by {eq}`eq-tn-consistency`.
3. Specimen bent or the area polycrystalline: the rigid assumption fails and no reconstruction is valid; bounded by {eq}`eq-tn-consistency` across pairs.
4. Target orbit outside the envelope: not a failure but an answer, and the most common useful output.
5. Target near the $\beta$-axis pole: ill-conditioned by $1/\cos\alpha$, bounded at $1.31$ within a real envelope.
6. One of the ten affected point groups with a polarity-sensitive purpose: irreducible from kinematic SAED; reported, not guessed.

## References

- De Graef, M., *Introduction to Conventional Transmission Electron Microscopy*, Cambridge University Press, DOI: 10.1017/CBO9780511615092 — stage geometry, diffraction rotation, Kikuchi navigation.
- Williams, D. B. and Carter, C. B., *Transmission Electron Microscopy*, Springer, DOI: 10.1007/978-0-387-76501-3 — indexing, tilt practice, calibration procedures.
- Wahba, G., *SIAM Review* **7** (1965) 409, DOI: 10.1137/1007077 — the two-vector attitude problem underlying the two-zone reconstruction.
- Hahn, Th. (ed.), *International Tables for Crystallography, Volume A*, IUCr/Springer, DOI: 10.1107/97809553602060000100 — point groups, Laue classes, stabilizer orders.
- Morawiec, A., *Orientations and Rotations*, Springer, DOI: 10.1007/978-3-662-09156-2 — rotation representations, symmetry orbits, tangent-space covariance.
