# Vectorized Miller Planes and Directions in PyTex

## Scope

This note records the scientific contract for the first-class PyTex Miller surface:

- scalar `MillerPlane` and `MillerDirection`,
- batch `MillerPlaneSet` and `MillerDirectionSet`,
- vectorized conversion between integer indices and cartesian direct or reciprocal vectors,
- family canonicalization, antipodal handling, and symmetry-equivalent expansion.

The design goal is to preserve exact integer input while making family operations explicit and
vectorized.

## Stored Indices And Bases
...
- row-wise GCD reduction,
- deterministic sign canonicalization by requiring the first non-zero component to be positive,
- antipodal family keys given by reduced-and-sign-canonicalized indices.
...
## Hexagonal Four-Index Conversions
...
- R. Hielscher and collaborators, *MTEX documentation*, <https://mtex-toolbox.github.io/>.
