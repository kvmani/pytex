...
\emph{Relationship to the existing note.}
\texttt{docs/tex/algorithms/orientation\_representations\_and\_plane\_direction\_construction.tex}
...
This note does not repeat them. It covers what that note does not: the two \emph{equal-volume}
charts (homochoric and cubochoric), the measure argument that motivates them, and the
inversion that recovers \((hkl)[uvw]\) indices \emph{from} an orientation.
