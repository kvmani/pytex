...
\subsection{The Reverse Inversion: Orientation From An Indexed Pattern}

Sections above invert \eqref{eq:tn-master} for the tilts. The same equation inverts for the
...
angles at which the pattern was recorded, \eqref{eq:tn-mode-a} gives $\mathbf{U}$ directly, and
...
\subsubsection{Joint determination of orientation and diffraction rotation}

The single-pattern inversion requires $\varphi_D$, and \eqref{eq:tn-residual-law} shows that an
...
Let patterns $i = 1, 2, \ldots$ be indexed at stage positions $(\alpha_i, \beta_i)$, with zone axes
$\hat{\mathbf{n}}_i$ and crystal-to-pattern rotations $\mathbf{R}_i$. The zone axes alone determine
$\mathbf{U}$ through \eqref{eq:tn-mode-b}, using no calibration. With $\mathbf{U}$ fixed, define

\begin{equation}
\mathbf{M}_i \;\equiv\; \mathbf{R}_{\text{stage}}(\alpha_i,\beta_i)\,\mathbf{U}\,\mathbf{R}_i^{\mathsf T}.
\label{eq:tn-residual}
\end{equation}

Substituting \eqref{eq:tn-mode-a} shows $\mathbf{M}_i = \mathbf{F}\mathbf{R}_z(\varphi_D)$ for every
$i$. Two consequences follow, and they are independent:

\begin{enumerate}
  \item \textbf{The diffraction rotation is read off, not fitted.} With $\mathbf{F} = \mathbf{I}$,
    $\varphi_D = \operatorname{atan2}\left((\mathbf{M}_i)_{21}, (\mathbf{M}_i)_{11}\right)$ exactly.
    The scatter of that value across patterns measures whether they agree on one instrument
    constant. Because $\varphi_D$ is an angle, the mean must be taken circularly: an arithmetic mean
    of $359^{\circ}$ and $1^{\circ}$ is $180^{\circ}$, which would be a spectacular way to report a
    calibration.
  \item \textbf{The beam-axis deviation is a residual no calibration can absorb.} Every
    $\mathbf{M}_i$ must fix $\hat{\mathbf{z}}_L$. The angle
    $\angle(\mathbf{M}_i\hat{\mathbf{z}}_L, \hat{\mathbf{z}}_L)$ is therefore orthogonal to the
    entire $\varphi_D$ degree of freedom, and is non-zero only if an input is wrong --- a mirrored
    stored pattern, a mis-indexed reflection, a reversed stage sign convention, or a bent specimen.
\end{enumerate}
...
\emph{any} value of the constant, not merely fitted to the best one.

\subsubsection{How a mirrored pattern presents}
...
in-plane axes \emph{and} reverses the sense of the derived pattern normal, since the normal is
...
\eqref{eq:tn-residual}. An improper matrix arriving at this surface means a mirror was composed in
