\section{Stereographic Kikuchi Maps And Zone-Axis Routing}

This note fixes the geometry, the combinatorics and the error analysis behind
\texttt{pytex.diffraction.kikuchi\_map}:
\texttt{StereographicKikuchiMap}, \texttt{KikuchiMapBand},
\texttt{KikuchiMapZoneAxis}, \texttt{KikuchiRoute},
\texttt{compute\_kikuchi\_map} and \texttt{plan\_kikuchi\_route}, with the figure
produced by \texttt{pytex.plotting.plot\_kikuchi\_map}.
...
\texttt{diffraction-kikuchi-map-zone-axis-tilt-angles}.

\subsection{The Band On The Sphere}
...
$\lambda = 0.02508$~\AA, a $2$~\AA{} spacing gives $2\theta_B = 0.72^\circ$. Two
consequences are worth separating explicitly, because they are routinely
conflated:

\begin{itemize}
  \item the \emph{widest} bands of a map come from the \emph{smallest} spacings,
        hence from high-index planes;
  \item the \emph{strongest} bands come from the largest spacings, hence from
        low-index planes, because $|F_{\mathbf{g}}|$ falls with $|\mathbf{g}|$.
\end{itemize}
...
\subsection{Why The Projection Must Be Stereographic}
...
is $0.99983$. That is why \texttt{pytex.diffraction.kikuchi} uses gnomonic
...
wrong one here. \texttt{plot\_kikuchi\_map} accepts either and defaults to
stereographic.

\subsection{Zone Axes As Exact Combinatorics}
...
without any metric tensor. Equation~\eqref{eq:zone-law} is an integer condition,
...
$\mathbf{u} \propto \mathbf{g}_1 \times \mathbf{g}_2$, and the \emph{order} of an
axis is the number of reflections in the map satisfying
equation~\eqref{eq:zone-law} for it. Order, not index, is the operationally
...
\paragraph{One band per plane trace.}
...
number an operator reads. \texttt{compute\_kikuchi\_map} therefore collapses each
family of collinear triples onto its lowest \emph{allowed} order. The
...
\texttt{include\_higher\_orders} restores the unfolded set for studies of
higher-order line positions, where the differing Bragg angles are the point.

\paragraph{Antipodal identification.}
A zone axis is a \emph{line}, not a direction: the beam traverses the crystal one
...
\subsection{Routing: The Band Is The Geodesic}
...
When no single band joins the endpoints, \texttt{plan\_kikuchi\_route} searches
the zone-axis network. Nodes are zone axes; an edge joins two axes that share at
least one band and are separated by no more than \texttt{max\_leg\_deg}; edge
...
\subsection{Why Several Short Hops Beat One Long One}
...
direction is therefore the intended one rotated about the \emph{starting} axis by
...
The miss scales with the sine of the \emph{hop length}. Split the excursion into
...
\texttt{DEFAULT\_ROUTE\_MAX\_LEG\_DEG} is $30^\circ$ for this reason.

Equation~\eqref{eq:tilt-miss} is the quantitative form of the argument in
\S11 of the TEM tilt-navigation foundation document, and it is verified against a
direct rotation simulation in tutorial~30.

\subsection{Intensities, And What The Map Does Not Model}

Band intensities use the Mott--Bethe electron structure factor of
\texttt{pytex.diffraction.scattering}, not the atomic-number proxy in
\texttt{pytex.diffraction.kinematic}. The proxy replaces $f_e(s)$ by $Z$, which is
...
job of \texttt{pytex.tem.navigation} and \texttt{pytex.tem.path}, which
\texttt{KikuchiRoute.describe} states explicitly. Finally, the search minimizes
travel rather than risk: equation~\eqref{eq:tilt-miss} would sometimes prefer a
longer path through more intersections, and that trade is not modelled.

\subsection{References}

\begin{itemize}
  \item N.~K. Levine, W.~L. Bell and G. Thomas, \emph{Further applications of
        Kikuchi diffraction patterns; electron diffraction pattern maps},
        J.~Appl.~Phys.\ \textbf{37} (1966) 2141. The original montaged maps.
  \item J.~W. Edington, \emph{Practical Electron Microscopy in Materials
        Science}, Monograph~2, Philips Technical Library (1975). Kikuchi maps as
        an operating technique.
  \item D.~B. Williams and C.~B. Carter, \emph{Transmission Electron
        Microscopy}, 2nd ed., Springer (2009), Ch.~19. Kikuchi line geometry and
        its use for tilting.
  \item A.~J.~C. Wilson and E. Prince (eds.), \emph{International Tables for
        Crystallography}, Vol.~C, 2nd ed., Kluwer (1999). Reciprocal-lattice
        conventions and the zone law.
  \item J.~P. Snyder, \emph{Map Projections --- A Working Manual}, USGS
        Professional Paper 1395 (1987), \S\S20--21. The azimuthal projections and
        what each preserves.
\end{itemize}
