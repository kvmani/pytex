...
\begin{itemize}
  ...
  \item discrete pole-figure inversion over an explicit orientation dictionary,
  \item contour pole-figure plotting from projected density grids,
  \item kernel-smoothed Bunge-section plotting for discrete ODF inspection.
\end{itemize}
...
\section{Discrete Pole-Figure Inversion}
...
\section{Plotting And Inspection}

PyTex uses the same discrete texture model for computation and plotting.

\begin{itemize}
  \item Pole-figure contours are rendered from a smoothed density grid in the chosen projection plane.
  \item Euler-space ODF contours are rendered from the weighted support directly.
  \item Classical Bunge sections are rendered as kernel-smoothed inspection views through the discrete support.
\end{itemize}
