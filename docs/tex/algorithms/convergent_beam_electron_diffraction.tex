\section{Convergent-Beam Electron Diffraction}
...
\emph{disc} in which each point corresponds to a different incident direction --- that is,
...
\texttt{pytex.diffraction.cbed}; the parallel-beam companion is
\texttt{docs/tex/algorithms/reciprocal\_space\_and\_kinematic\_spots.tex}.

\subsection{Disc Geometry}
...
\texttt{pytex.diffraction.kinematic}, so the two engines share one convention.

Two consequences follow directly from \eqref{eq:cbed-s}.

\paragraph{The fringes are straight and perpendicular to \(\mathbf{g}\).} \(s_g\) is
\emph{linear} in the tilt, with gradient \(-\mathbf{g}_\perp\). Lines of constant \(s_g\)
are therefore straight lines perpendicular to \(\mathbf{g}_\perp\), which is exactly what a
Kossel--M\"ollenstedt disc looks like.

\paragraph{The disc radius.} A tilt \(\boldsymbol{\theta}\) displaces the diffracted beam
...
\paragraph{The two regimes.} Neighbouring discs touch when
\(2\alpha/\lambda = |\mathbf{g}|_{\min}\). Below that the discs are separated
(\emph{Kossel--M\"ollenstedt}) and each is an independent rocking curve; above it they
overlap (\emph{Kossel}) and the overlap regions carry interference between beams. Only the
first regime supports fringe counting, which is why
\texttt{CBEDPattern.is\_kossel\_moellenstedt} exists and why its
\texttt{describe()} states the regime before anything else.

\paragraph{Where the exact Bragg condition sits.} A disc is centred at
...
\subsection{The Two-Beam Rocking Curve}
...
the thickness increases, the Pendell\"osung oscillation, and that is what makes
...
\subsection{The Extinction Distance And Its Absolute Scale}
...
with \(V_{c}\) the unit-cell volume and \(F_{g}\) the electron structure factor \emph{in
...
in \aa ngstr\"om. The numerator \(Z - f_{x}\) is the nuclear charge screened by the electron
...
into \eqref{eq:mott-bethe} returns exactly \(\sum_i a_i e^{-b_i s^{2}}\), so the same fitted
coefficients \emph{are} the electron scattering-factor parameters and the constant
...
\paragraph{Validation.} Against Williams and Carter (Table 23.1), aluminium at 100~kV:
...
\hline
tabulated (\AA) & 556 & 673 & 1057 \\
PyTex (\AA)     & 555 & 664 & 1063 \\
\hline
\end{tabular}
\end{center}
...
reason CBED practice \emph{measures} \(\xi_{g}\) rather than tabulating it, as the next
section does.

\subsection{Thickness Determination}

The minima of \eqref{eq:two-beam} occur where \(t\,s_{\mathrm{eff}} = n\) for integer
...
Plotting \((s_n/n)^{2}\) against \(1/n^{2}\) therefore gives a \emph{straight line} whose
...
distance. This is the linearization of Kelly \emph{et al.} (1975), implemented as
\texttt{thickness\_from\_fringe\_minima}.

\paragraph{Algorithm.}
\begin{enumerate}
  \item Read the dark-fringe positions \(s_n\) from one disc, on the branch that shows the
        more fringes.
  \item Assign consecutive integers \(n_0, n_0+1, \dots\) to them, innermost first.
  \item Fit \eqref{eq:kelly} by least squares; require intercept \(> 0\) and slope \(< 0\),
        which is what makes the result physical.
  \item If \(n_0\) is unknown, repeat over candidate \(n_0\) and take the assignment with
        the best \(R^{2}\) among the physical fits.
\end{enumerate}

\paragraph{The known failure.} The order of the innermost \emph{visible} minimum is rarely
...
\subsection{Higher-Order Laue Zones}

A zone-axis pattern is blind to the lattice repeat \emph{along the beam}, because every
...
\begin{itemize}
  \item \textbf{Many-beam dynamical coupling.} Each disc is computed in its own two-beam
        approximation, so the discs of one simulated pattern are not mutually consistent and
        their relative intensities are not meaningful. At a zone axis --- where many
        reflections are simultaneously excited --- this is the two-beam picture at its worst.
  \item \textbf{Absorption.} The fringes therefore do not decay with thickness as they do in
        practice.
  \item \textbf{HOLZ lines} inside the bright-field disc, the sharp features used for
        lattice-parameter metrology. Only the ring radii are given.
  \item \textbf{Diffraction-group symmetry determination.} The symmetry within and between
        discs determines the diffraction group and hence the point group \emph{including} the
        presence of a centre of symmetry, which Friedel's law hides from kinematic SAED. That
        determination needs full dynamical intensities and is not attempted;
        \texttt{CBEDPattern.describe()} says so explicitly.
\end{itemize}

\subsection{Normative And Informative Sources}

\begin{itemize}
  \item P. M. Kelly, A. Jostsons, R. G. Blake and J. G. Napier, ``The determination of foil
        thickness by scanning transmission electron microscopy'', \emph{Physica Status Solidi
        (a)} \textbf{31} (1975) 771--780 --- the linearization \eqref{eq:kelly}.
  \item D. B. Williams and C. B. Carter, \emph{Transmission Electron Microscopy}, 2nd ed.,
        Springer, 2009 --- two-beam theory, extinction distances (Table 23.1), and CBED
        practice.
  \item M. De Graef, \emph{Introduction to Conventional Transmission Electron Microscopy},
        Cambridge University Press, 2003 --- the Howie--Whelan equations and the
        scattering-factor conventions.
  \item J. W. Steeds, ``Convergent beam electron diffraction'', in \emph{Introduction to
        Analytical Electron Microscopy}, Plenum, 1979 --- the diffraction-group method not
        implemented here.
  \item N. F. Mott and H. S. W. Massey, \emph{The Theory of Atomic Collisions}, 3rd ed.,
        Oxford, 1965 --- the Mott--Bethe relation \eqref{eq:mott-bethe}.
  \item P. A. Doyle and P. S. Turner, ``Relativistic Hartree-Fock X-ray and electron
        scattering factors'', \emph{Acta Crystallographica A} \textbf{24} (1968) 390--397 ---
        the parametrization family behind the tabulated coefficients.
\end{itemize}
