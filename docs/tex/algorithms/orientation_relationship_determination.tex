\section{Determining An Orientation Relationship From Measured Orientations}

The index-correspondence note fixes what an orientation relationship \emph{does} once its
rotation $\mathbf{R}$ is known. This note fixes how $\mathbf{R}$ is \emph{recovered} from
measured parent and child orientations, and how it is then named and stated. The algorithm is
implemented in \texttt{pytex.core.transformation.characterize\_orientation\_relationship}.
...
\label{eq:or-measured-pair}
\end{equation}

Equation~\eqref{eq:or-measured-pair} has one implementation in the library; no downstream surface
re-derives the placement of the transpose.

\subsection{Two Ambiguities, One Group Structure}
...
Second, distinct child grains generally form through distinct \emph{variants}. A variant is the
...
\label{eq:or-double-coset}
\end{equation}

where $G_{p}$ and $G_{c}$ are the parent and child point groups as sets of proper rotations. The
variant operation lies inside $G_{p}$, so~\eqref{eq:or-double-coset} contains every description
...
\subsection{Seeding Without A Nominal Relationship}
...
Equation~\eqref{eq:or-seed} returns the disorientation description of the relationship that pair
shows; every other pair then has an equivalent description near it.

\paragraph{Why exactly one pair is reduced.} Reducing all pairs independently and averaging the
representatives is incorrect. The maximiser in~\eqref{eq:or-seed} is not unique when
...
\subsection{Symmetry-Aware Rotation Averaging}
...
Convergence is declared when the assignment set in~\eqref{eq:or-align} repeats, the mean being a
...
\subsection{Naming: Symmetry-Reduced Catalog Distance}
...
\label{eq:or-catalog-distance}
\end{equation}

the catalog itself being selected from the two crystal systems through one dispatch table. The
separations~\eqref{eq:or-catalog-distance} must resolve are fixed crystallography: within the
...
\subsection{Stating The Relationship}
...
\paragraph{Non-uniqueness of the statement.} A rotation generally satisfies several exact
...
them. Which clause the literature quotes is determined by the two \emph{structures} --- their
...
\begin{itemize}
  \item Pairs must be row-matched and share a specimen frame; grain-mean orientations are
        assumed, and the method does not itself perform grain segmentation.
  \item The cubic-to-cubic catalog assumes an fcc-to-bcc transformation, because point-group
        symmetry cannot distinguish an fcc phase from a bcc one. An explicit catalog must be
        supplied when that assumption fails.
  \item The parallelism search is bounded; a relationship defined by higher-index parallelisms
        reports no statement rather than an invented one.
  \item Validation is synthetic. Measured-EBSD fixtures and a MTEX \texttt{calcParent2Child}
        parity comparison remain outstanding, and no PyTex document claims that parity.
\end{itemize}
