...
(\emph{deficiency}) in the direct disc and bright (\emph{excess}) at the same incident tilts in
the disc of \(\mathbf{g}\) itself. Nothing beyond \eqref{eq:dyn-selection} is approximated, so
line positions are exact to the accuracy of \(s_g\); \texttt{pytex} checks them by asking the
dynamical module for \(s_g\) at points on the line and requiring zero to \(10^{-15}\).

\paragraph{Sharpness.} The rocking curve has its first zero at \(|s| = 1/t\), and \(s\) varies
...
thickness --- HOLZ metrology wants a \emph{thick} specimen, the opposite of the usual thin-foil
instinct.

\paragraph{The metrology, and the trap in it.} Scaling the lattice by \(1+\varepsilon\) shrinks
every \(\mathbf{g}\) by the same factor. Substituting into \eqref{eq:holz-line},
...
and the wavelength enters the same term with the opposite sign. Setting \(\lambda \to
\lambda(1+\varepsilon)\) therefore cancels a lattice strain \(\varepsilon\) exactly, at every
reflection simultaneously. \emph{A fractional change in lattice parameter and a fractional
...
lattice parameter is a measurement of the high-tension supply. \texttt{pytex} asserts the
degeneracy to \(10^{-16}\) rather than describing it.

\paragraph{Why intersections are measured, not lines.} For nickel down \([001]\) at 200~kV in a
\(1000\)~\AA{} foil, the best single line moves \(0.059\)~mrad per unit strain against a
...
\texttt{HOLZLinePattern.intersections} reports the amplification explicitly.

\subsection{Diffraction Groups}
...
is a symmetry only in combination with the reciprocity theorem. The map \(S \mapsto (S|_\perp,
\text{tag})\) is a homomorphism onto a subgroup of \(G_2 \times \mathbb{Z}_2\) with \(G_2\) one
...
31, obtained by construction rather than transcription, and \texttt{pytex} verifies the count
and the membership against the published set.

\paragraph{The two observables.} A tagged element requires reciprocity, which relates a point in
...
two-fold alone (\(1_R\)); and \(\bar{6}m2\) down its three-fold shows a \emph{six}-fold direct
disc over a \(3m\) whole pattern (\(3m1_R\)).

\paragraph{The centre of symmetry.} The element \(2_R\) requires an operator acting as \(-1\) on
the beam direction \emph{and} as \(-1\) on the transverse plane, which is the inversion and
...
\paragraph{What \(\pm\mathbf{G}\) is, and is not.} Buxton's \(\pm\mathbf{G}\) observation
compares the \(+\mathbf{g}\) and \(-\mathbf{g}\) \emph{dark-field} discs, each recorded with its
own reflection at the Bragg condition --- two exposures at different specimen tilts, related by
reciprocity. It is \emph{not} a two-fold rotation of a single zone-axis pattern. Taking it to be
...
centrosymmetric crystal too, and numerically the residual \emph{grows} with the beam set for
centric and acentric structures alike --- so it is physics, not truncation. \texttt{pytex}
...
\paragraph{The worked case.} Down \([001]\), zincblende \(\bar{4}3m\) has diffraction group
...
polar structure with the centrosymmetry verdict \emph{false}. Confine the beam set to the
zeroth Laue zone and the same crystal reports the four-fold whole-pattern symmetry of a
centrosymmetric one: the projected potential of zincblende down \([001]\) \emph{is}
centrosymmetric. That is the single most important caveat in CBED symmetry work, and it is
enforced rather than described --- \texttt{symmetry\_observations} refuses a projection
calculation unless asked a second time.

\paragraph{Not implemented.} Buxton's table also lists dark-field and \(\pm\mathbf{G}\)
