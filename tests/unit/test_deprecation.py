...
def test_deprecation_message_shape() -> None:
    message = deprecation_message(
        "pytex.old_name", since="0.2.3", removal="0.4.0", replacement="pytex.new_name"
    )
    ...
    bare = deprecation_message("pytex.old_name", since="0.2.3", removal="0.4.0")
    assert "Use" not in bare


def test_decorator_warns_and_preserves_behavior() -> None:
    @deprecated(since="0.2.3", removal="0.4.0", replacement="new_add")
...
def test_warn_deprecated_implicates_caller() -> None:
    def deprecated_surface() -> None:
        warn_deprecated("deprecated_surface", since="0.2.3", removal="0.4.0", stacklevel=3)
